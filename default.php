...
class KudosPlugin extends Gdn_Plugin {
  ...
  public function DiscussionController_BeforeDiscussionRender_Handler($Sender) {
    ...
    public function DiscussionController_BeforeCommentDisplay_Handler($Sender) {
        if (!C('Plugins.Kudos.Delete')) {
            return;
        }
        $points = C('Plugins.Kudos.DeleteNumber') * -1;
        if ($this->KudosModel->CommentRate($Sender->EventArguments['Object']->CommentID) <= $points) {
            echo '<!--';
        }
    }
  ...
  public function FormatKudos($DiscussionID, $CommentID = false)
  {
  	...
  	if($CommentID) {
            $Kudos = $this->KudosModel->GetCommentKudos($CommentID);
        }
  	...
  	if($this->loved($Kudos)) {
            ...
            $Toolbar .= '<a href="'.Url('discussion/kudos/'.$DiscussionID.'/1/'.$CommentID).'">+1</a>&nbsp;';
        }
  	if($this->hated($Kudos)) {
            $Toolbar .= '<b>-1</b>';
        }
  	else {
            $Toolbar .= '<a href="'.Url('discussion/kudos/'.$DiscussionID.'/2/'.$CommentID).'">-1</a>';
        }
